...
{
    constexpr auto driver_name = "a555.sys";
	...
	bool CallKernelFunction( HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments )
	{
        ...
        HMODULE hWin32u = LoadLibraryA( xorstr_( "win32u.dll" ) );

		if ( !hWin32u )
			return false;

		FARPROC win32u_routine
            = GetProcAddress( hWin32u, xorstr_( "NtTokenManagerConfirmOutstandingAnalogToken" ) );
		...
		const uint64_t dxgk_routine
            = GetKernelModuleExport( device_handle, utils::GetKernelModuleAddress( "dxgkrnl.sys" ), xorstr_( "NtTokenManagerConfirmOutstandingAnalogToken" ) );
		...
		//for ( int i = 0; i < sizeof( dxgk_original ); i++ )
		//{
		//	//printf( " %02X ", dxgk_original[i] );
		//}
