...
{
	//std::count << "[<] Loading vulnerable driver" << std::endl;

	// Does it already exist for some reason? ie: application crashed before previously

	HANDLE intel_driver_handle
		= CreateFileW( L"\\\\.\\Nal", GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );

	if ( intel_driver_handle )
	{
		CloseHandle( intel_driver_handle );
	...
	{
		//std::count << "[-] Failed to get temp path" << std::endl;
		return nullptr;
	}
	...
	{
		//std::count << "[-] Failed to create vulnerable driver file: " << driver_path << std::endl;
		return nullptr;
	}

	if (!service::RegisterAndStart(driver_path))
	{
		//std::count << "[-] Failed to register and start service for the vulnerable driver" << std::endl;
		std::remove(driver_path.c_str());
		return nullptr;
	}
...
{
	//std::count << "[-] Unloading vulnerable driver" << std::endl;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		//std::count << "[-] Failed to translate virtual address 0x" << reinterpret_cast<void*>(address) << std::endl;
		return false;
	}
	...
	{
		//std::count << "[-] Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address) << std::endl;
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		//std::count << "[!] Failed to unmap IO space of physical address 0x" << reinterpret_cast<void*>(physical_address) << std::endl;
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	//printf( "[+] CallKernelFunction( ExAllocatePool )\n" );
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address)
{
	...
	//printf( "[+] CallKernelFunction( ExFreePool )\n" );
